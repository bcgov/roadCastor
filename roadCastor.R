...
defineModule(sim, list(
  ...
  parameters = rbind(
    ...
    defineParameter("nameCostSurfaceRas", "character", "rast.base_cost_surface", NA, NA, desc = "Name of the cost surface raster"),
    defineParameter("nameTimeSurfaceRas", "character", "9999", NA, NA, desc = "Name of time surface raster"),
    defineParameter("gradePenaltyLimit", "numeric", -9999, NA, NA, desc = "Name of the cost surface raster"),
    defineParameter("nameElevationRaster", "character", "rast.bc_ha_dem", NA, NA, desc = "Name of the digitial elevation raster in metres"),
    ...
    defineParameter("saveHaulRoutes", "logical", FALSE, NA, NA, "Should the roads be saved in roadlist table? only for mst and lcp since pre already does this"),
    defineParameter("loopBoundaryRoads", "logical", FALSE, NA, NA, "Should boundary pixels that are also perm roads be looped into the exits"),
  ...
  inputObjects = bind_rows(
    ...
    expectsInput(objectName = "sinkLocations", objectClass = "data.table", desc = "The x, y location in BC Albers (epsg: 3005) that define end points for the road network (log dumps, support centres, mills/processing facilities)", sourceURL = NA),
...
Init <- function(sim) {
  ...
  if(P(sim)$roadMethod %in% c('mst', 'lcp')){ #the other methods
    dbExecute(sim$castordb, "CREATE TABLE IF NOT EXISTS roadslist ( landing integer,  road character, cyc_time numeric )")
...
saveRoads<-function(sim){
  dbExecute(sim$castordb, "delete from roadedges;")

  dbBegin(sim$castordb)
  rs<-dbSendQuery(sim$castordb, "INSERT INTO roadedges ('to' , 'from', 'weight', 'time')
                      values (:to, :from, :weight, :time);", sim$edges.weight)
  dbClearResult(rs)
...
getCostSurface<- function(sim){
  ...
  if(!is.null(sim$boundaryInfo)){
    ...
    costSurf<-terra::rast(RASTER_CLIP2(tmpRast =paste0('temp_', sample(1:10000, 1)),
                           ...
                           where_clause =  paste0 (sim$boundaryInfo[[2]], " in (''", paste(sim$boundaryInfo[[3]], sep = "' '", collapse= "'', ''") ,"'')"),
                           conn=sim$dbCreds))
    if(ext(rds) == ext(costSurf)){
      costSurf[rds[] == 0]<-0
    }else{
      stop(message("costSurface not same extent as sim$road.type"))
    }

    sim$costSurface<- costSurf
    sim$costSurface[sim$costSurface[]==-9999]<-NA
    sim$costSurface[sim$costSurface == 0]<- 0.00000000001 #giving some weight to roaded areas
    terra::writeRaster(sim$costSurface, file=paste0 (sim$scenario$name, "_", sim$boundaryInfo[[3]][[1]],"_cost_new_rds.tif"), overwrite=TRUE)

    sim$timeSurface<-terra::rast(RASTER_CLIP2(tmpRast =paste0('temp_', sample(1:10000, 1)),
                                       srcRaster=P(sim, "nameTimeSurfaceRas", "roadCastor"),
                                       clipper=sim$boundaryInfo[[1]],
                                       geom=sim$boundaryInfo[[4]],
                                       where_clause =  paste0 (sim$boundaryInfo[[2]], " in (''", paste(sim$boundaryInfo[[3]], sep = "' '", collapse= "'', ''") ,"'')"),
                                       conn=sim$dbCreds))
    if(!(ext(rds) == ext(sim$timeSurface))){
      stop(message("timeSurfaceRas not same extent as sim$road.type"))
    }
...
updateRoadsTable <- function(sim){
  ...
  if(P(sim)$saveHaulRoutes){
    browser()
    message('...getting cycle_time')
    sim$g_time <- cppRouting::makegraph(sim$edges.weight[,c(1:2,4)], directed=F, coords = sim$node.coords) #rebuild the cppRouting graph
    rs.roadslist<-cppRouting::get_multi_paths(Graph = sim$g_time, from = sim$roadSourceID, to= sim$landings[sim$landings %in% sim$nodes], long =F)
    sim$roadslist<-rbindlist(lapply(names(rs.roadslist[[1]]) , function(x){
      data.table(landing = as.integer(x), road = toString(rs.roadslist[[1]][[x]]))
    }))
    cyc_time<-cppRouting::get_distance_matrix(Graph = sim$g_time, from = sim$roadSourceID, to= sim$landings[sim$landings %in% sim$nodes])  + 1.3
    cyc_time<-data.table(time=t(cyc_time))
    setnames(cyc_time, "cyc_time")
    # store roadslist in castordb
    message("store road list in castordb")
    dbBegin(sim$castordb)
    rs<-dbSendQuery(sim$castordb, "INSERT INTO roadslist (landing, road, cyc_time )
                      values (:landing, :road, :cyc_time)", cbind(sim$roadslist,cyc_time))
    dbClearResult(rs)
    dbCommit(sim$castordb)
    message('...vacuum')
    #Vacuum
...
setGraph<- function(sim){
  ...
  sim$roadSourceID<-terra::ncell(sim$ras) + 1
  ...
  weight<-data.table(cost = sim$costSurface[], maxspeed = sim$timeSurface[], type = sim$road.type[]) #transpose then vectorize which matches the same order as adj
  ...
  setnames(edges.w1, c("from", "to", "w1",  "maxspeed", "type")) #reformat
  edges.w2<-data.table::setDT(merge(x=edges.w1, y=weight[,c("id", "cost.lyr.1", "maxspeed.lyr.1")], by.x= "to", by.y ="id"))#merge in the weights to a cost surface
  setnames(edges.w2, c("from", "to", "w1", "maxspeed","type", "w2", "maxspeed2")) #reformat
  edges.w2$weight<-(edges.w2$w1 + edges.w2$w2)/2 #take the average cost between the two pixels

  #------prepare the grade penalty
  if(!(P(sim)$gradePenaltyLimit == -9999)){
    #check to see if elevation is in the db
    check_elv<-dbGetQuery(sim$castordb, "select max(elv) as maxelv from pixels;")
    if(check_elv$maxelv == 0){
      elv <- data.table(elv = RASTER_CLIP2(tmpRast = paste0('temp_', sample(1:10000, 1)),
                                           srcRaster = P(sim, "nameElevationRaster", "roadCastor"),
      ...
      elv<-data.table(dbGetQuery(sim$castordb, "select pixelid, elv from pixels order by pixelid;"))
    }
    #------Assign grade penalty
    edges.w2<-merge(edges.w2, elv, by.x= "to", by.y ="pixelid")
    setnames(edges.w2, "elv", "elv_to")
    edges.w2<-merge(edges.w2, elv, by.x= "from", by.y ="pixelid")
    setnames(edges.w2, "elv", "elv_from")

    edges.w2<-edges.w2[, grade:=(abs(elv_from-elv_to)/200)*100][grade <=5,penalty:=0][grade > 5 & grade <= P(sim)$gradePenaltyLimit, penalty:=0.2*(grade-5)][grade > P(sim)$gradePenaltyLimit, penalty:=P(sim)$gradePenaltyLimit][ penalty > 0, weight:=weight*(1 + penalty)]
    #edges.w2<-edges.w2[penalty < 2, ] # remove edges too steep to build
    #edges.w2<- edges.w2[grade <= 6, time:=0.004][grade > 6 & grade <= 8, time:=0.005][grade > 8 & grade <= 10, time:=0.0066][grade > 10 & grade <= 12, time:=0.01][grade > 12 & grade <= 18, time:=0.0134][grade > 18, time:=0.0144]
    #time is simply (length of road in pixel in km/design speed km/hr)*2 for there and back

    pixel_rd_density<-0.12
    #edges.w2<- edges.w2[!is.na(grade) & (is.na(maxspeed) | maxspeed == -9999), maxspeed := 0.001]
    edges.w2<- edges.w2[is.na(maxspeed) | maxspeed == -9999, maxspeed := 0.001]
    edges.w2<- edges.w2[is.na(maxspeed2) | maxspeed2 == -9999, maxspeed := 0.001]

    #if(FALSE){
    edges.w2<- edges.w2[grade <= 6, time:=(pixel_rd_density/maxspeed)*2][grade > 6 & grade <= 8 & maxspeed >= 50, time:=(pixel_rd_density/50)*2]
    edges.w2<- edges.w2[grade > 6 & grade <= 8 & maxspeed < 50, time:=(pixel_rd_density/maxspeed)*2][grade > 8 & grade <= 10 & maxspeed >= 40, time:=(pixel_rd_density/40)*2]
    edges.w2<- edges.w2[grade > 8 & grade <= 10 & maxspeed < 40, time:=(pixel_rd_density/maxspeed)*2][grade > 10 & grade <= 12 & maxspeed >= 35, time:=(pixel_rd_density/35)*2]
    edges.w2<- edges.w2[grade > 10 & grade <= 12 & maxspeed < 35, time:=(pixel_rd_density/maxspeed)*2][grade > 12 & grade <= 16 & maxspeed >= 30, time:=(pixel_rd_density/30)*2]
    edges.w2<- edges.w2[grade > 12 & grade <= 16 & maxspeed < 30, time:=(pixel_rd_density/maxspeed)*2][grade > 16 & maxspeed >= 20, time:=(pixel_rd_density/20)*2][grade > 16 & maxspeed > 1 & maxspeed < 20, time:=(pixel_rd_density/maxspeed)*2]
    #}
    #edges.w2<- edges.w2[, time:=(pixel_rd_density/maxspeed)*2]

    edges.w2<- edges.w2[time == 240 & w1 < 1 & w2 < 1, time:=(pixel_rd_density/30)*2]
    edges.weight<-edges.w2[complete.cases(edges.w2[, c("from", "to", "weight", "time")]), c("from", "to", "weight", "time")] #get rid of NAs caused by barriers. Drop the w1 and w2 costs.

  }else{
    #------get the edges list
    edges.weight<-edges.w2[complete.cases(edges.w2[, c(1:2, 6)]), c(1:2, 6)] #get rid of NAs caused by barriers. Drop the w1 and w2 costs.
    edges.weight$time<-edges.weight$weight
  }
 ...
 if(P(sim)$loopBoundaryRoads){
  ...
  #Take care of sinkLocations
  sinkLocales<-cellFromXY(sim$ras, sim$sinkLocations)
  sim$sinkLocations$cell<-sinkLocales
  sinkLocales<-sinkLocales[!is.na(sinkLocales)]
  sim$sinkLocations<-sim$sinkLocations[!is.na(cell),]
  sim$sinkLocations<-sim$sinkLocations[,id:=seq_len(.N)]
 # create Tavle if not exists sinkLocales
  dbExecute(sim$castordb, "CREATE TABLE IF NOT EXISTS roadsinks (  id integer, cell integer, x numeric, y numeric)")
  dbBegin(sim$castordb)
  rs<-dbSendQuery(sim$castordb, "INSERT INTO roadsinks(id, cell, x ,y)
                      values (:id, :cell, :xcoord, :ycoord);", sim$sinkLocations)
  dbClearResult(rs)
  dbCommit(sim$castordb)
  #Check
  if(length(sinkLocales) == 0){
    stop("None of the sinkLocations are within the area of interest")
  }

  sinks<-data.table(from = sinkLocales, to = sim$roadSourceID, weight = 0.0000001, time =0)
  edges.weight<-rbind(edges.weight, sinks)
  ...
  sim$g<-cppRouting::makegraph(edges.weight[,1:3], aux = edges.weight$time, directed=F)
  #sim$g<-cppRouting::cpp_simplify(sim$g) #Removed nodes of interest see Issue #348

  graph.df<-cppRouting::to_df(sim$g)
  graph.df$time<-sim$g$attrib$aux
  ...
  dbExecute(sim$castordb, "CREATE TABLE IF NOT EXISTS roadedges ( 'to' integer, 'from' integer,  'weight' numeric, 'time' numeric )")
  dbBegin(sim$castordb)
  rs<-dbSendQuery(sim$castordb, "INSERT INTO roadedges ('to' , 'from', 'weight', 'time')
                      values (:to, :from, :dist, :time);", graph.df)
  dbClearResult(rs)
  dbCommit(sim$castordb)
  ...
  coords<-coords[is.na(x), x  := mean(sim$sinkLocations$xcoord)][is.na(y), y  := mean(sim$sinkLocations$ycoord)]
  ...
  ###optional update the landing location to be near roaded areas?
  new_landings<-dbGetQuery(sim$castordb, "SELECT blockid, pixelid
FROM (select pixelid, blockid, ROW_NUMBER() OVER(
            PARTITION BY blockid
            ORDER BY roadtype NULLS LAST, elv NULLS LAST
        ) as row_num from pixels where blockid > 0) as subquery_alias
WHERE
    row_num = 1;")

  dbBegin(sim$castordb)
  rs<-dbSendQuery(sim$castordb, 'UPDATE blocks SET landing = :pixelid WHERE blockid = :blockid', new_landings )
  dbClearResult(rs)
  dbCommit(sim$castordb)
...
getGraph<- function(sim){
  ...
  sim$g<-cppRouting::makegraph(sim$edges.weight[,1:3], directed=F, coords = sim$node.coords) #coordinates help find paths
...
setEdges<- function(sim){ #Updates the graph to account for the new roads
  message("setEdges: updating graph")
  browser()
  ...
  if(time(sim) == 1){
    ...
    sim$edges.weight[to %in% sim$paths.v & from %in% sim$paths.v, time := (1.25/50)*2]

    sim$edges.weight[to %in% sim$roadSegs & from %in% sim$roadSegs & time > 1, weight := 0.0000001]
    sim$edges.weight[to %in% sim$roadSegs & from %in% sim$roadSegs & time > 1, time := (1.25/60)*2]


  }else{
    sim$edges.weight[to %in% sim$paths.v & from %in% sim$paths.v, weight := 0.0000001] #assign a low cost to the newly made road
    sim$edges.weight[to %in% sim$paths.v & from %in% sim$paths.v, time := (1.25/35)*2]

    sim$edges.weight[to %in% sim$roadSegs & from %in% sim$roadSegs & time > 1, weight := 0.0000001]
    sim$edges.weight[to %in% sim$roadSegs & from %in% sim$roadSegs & time > 1, time := (1.25/40)*2]
  }
#------make the cppRouting graph object
  sim$g <- cppRouting::makegraph(sim$edges.weight[,1:3], directed=F, coords = sim$node.coords) #rebuild the cppRouting graph
...
mstSolve <- function(sim){
  ...
  if(nrow(sim$harvestPixelList)>0){
    #reassign the landing to the area being harvested
    landings.change<-sim$harvestPixelList[sim$harvestPixelList[, .I[which.min(dist)], by=blockid]$V1, c("blockid", "pixelid")]
    setnames(landings.change, c("blockid", "landing"))

    dbBegin(sim$castordb)
      rs<-dbSendQuery(sim$castordb, 'UPDATE blocks SET landing = :landing WHERE blockid = :blockid', landings.change )
      dbClearResult(rs)
    dbCommit(sim$castordb)

    sim$landings <-landings.change[landing %in% sim$nodes, ]$landing
    ...
    landing.cell <- landings.change[landing %in% sim$nodes, ]
  ...
  weights.closest.rd <- cppRouting::get_distance_matrix(Graph=sim$g,
                                  from=landing.cell$landing,
                                  to=sim$roadSourceID,
                                  allcores=FALSE)
  edge.list <- data.table(from= landing.cell$landing, to = sim$roadSourceID, weight = weights.closest.rd[,1])

  #------get the edge list between the closest landings
  message('...get nearest neighbours')
  #browser()
  if(nrow(landing.cell) > 1){
    nnlandings <- RANN::nn2(terra::xyFromCell(sim$ras, landing.cell$landing), k =2)
    edge.list.inner <- data.table(from= landing.cell$landing, id=nnlandings$nn.idx[,1], idnn =nnlandings$nn.idx[,2] )
  ...
  if(nrow(paths.matrix) > 0){
    ...
    if(nrow(paths.matrix.tothers) > 0){
      #paths.toRoadSourceID<-cppRouting::get_multi_paths(Graph = sim$g, from = sim$roadSourceID, to=paths.matrix[V2==sim$roadSourceID,"V1"]$V1, long =F)
     # roadslist.toRoadSourceID<-rbindlist(lapply(names(paths.toRoadSourceID[[1]]) , function(x){
      #       data.table(landing = as.integer(x), road = toString(paths.toRoadSourceID[[1]][[x]]))
      #   }))
      roadslist.toRoadSourceID<-cppRouting::get_multi_paths(Graph = sim$g, from = sim$roadSourceID, to=paths.matrix[V2==sim$roadSourceID,"V1"]$V1, long =T)
      toRoadSourceID<-unique(as.integer(roadslist.toRoadSourceID$node))

      #paths.toOthers<-cppRouting::get_path_pair(Graph = sim$g, from = paths.matrix.tothers$V2, to=paths.matrix.tothers$V1 , algorithm = "NBA", constant = 110/0.06, long =F)
      #roadslist.toOthers<-rbindlist(lapply(1:length(paths.toOthers) , function(x){
      #  data.table(path_name = names(paths.toOthers[x]), road = toString(paths.toOthers[x][[1]]))
      #}))
      #roadslist.toOthers<-roadslist.toOthers[,c("from", "to") := tstrsplit(path_name, "_")][, to:=as.integer(to)][, from:=as.integer(from)]
      roadslist.toOthers<-cppRouting::get_path_pair(Graph = sim$g, from = paths.matrix.tothers$V2, to=paths.matrix.tothers$V1 , algorithm = "NBA", constant = 110/0.06, long =T)
      toOthers<-unique(as.integer(roadslist.toOthers$node))
    ...
    #------Clean up
    rm(weights.closest.rd,edge.list,edges.all,gi.mst,paths.matrix,toRoadSourceID,alreadyRoaded)
    gc()
  }
  }
  return(invisible(sim))
...
getRoutes<-function(sim){ #for graphs using cppRouting
  message("getRoutes")
  landing.cell <-data.table(landing = sim$landings)[!(landings %in% sim$perm.roads$pixelid),] #remove landings on permanent roads
  sim$roadSegs <-unique(as.integer(cppRouting::get_multi_paths(Graph = sim$g, from = sim$roadSourceID, to = landing.cell$landing, long =T )$node))
  alreadyRoaded <-dbGetQuery(sim$castordb, paste0("SELECT pixelid from pixels where roadyear is not null and pixelid in (",paste(sim$roadSegs, collapse = ", "),")"))
...
preSolve<-function(sim){
  ...
  if(exists("histLandings", where = sim)){
    ...
    targets <- unique(c(as.character(cellFromXY(raster(sim$ras), SpatialPoints(coords = as.matrix(sim$histLandings[,c(2,3)]), proj4string = CRS("+proj=aea +lat_1=50 +lat_2=58.5 +lat_0=45 +lon_0=-126 +x_0=1000000 +y_0=0 +datum=NAD83
                      ...
                      dbGetQuery(sim$castordb, "SELECT landing FROM blocks;")$landing)) #Remove landings on permanent roads

  }else{
    #TODO: get the centroid instead of the maximum pixelid? This may involve having to convert to vector?
    targets <- unique(dbGetQuery(sim$castordb, "SELECT landing FROM blocks;")$landing) #Remove landings on permanent roads
  }
  ...
  cyc_time<-cppRouting::get_distance_pair(Graph = sim$g, from = rep(sim$roadSourceID,length(sim$landings[sim$landings %in% sim$nodes]) ), to= sim$landings[sim$landings %in% sim$nodes], aggregate_aux = TRUE) + 1.3
...
.inputObjects <- function(sim) {
  ...
  if(!suppliedElsewhere("sinkLocations", sim)){
    sim$sinkLocations<-rbindlist(list(data.table(xcoord = c(1322373.9568, 	1324957.4209, 	1322206.3159, 	1322295.259, 	1333548.6163, 	1322386.0292, 	1272798.8327, 	1273207.7498, 	1264993.8579, 	1311793.8064, 	1480577.0932, 	1482285.787, 	1409426.5902, 	1408130.0774, 	1214515.3307, 	1057796.9663, 	1061901.4467, 	1204737.5417, 	1682383.6918, 	1004904.1387, 	1032911.0686, 	1033879.7161, 	1035494.8789, 	1017403.4648, 	1051715.6828, 	1044380.036, 	1040322.5376, 	1049005.8341, 	1478060.3851, 	1612394.1342, 	1599660.0853, 	1600537.2354, 	1454059.9024, 	1442556.3237, 	1167836.9334, 	1165554.2276, 	1273921.4228, 	1277767.5099, 	1287853.034, 	1291996.381, 	1292727.8541, 	1292825.9666, 	1308343.3609, 	1491553.0493, 	1490888.3662, 	1071568.3786, 	1073681.1968, 	1173856.8824, 	1732441.1482, 	1694348.9341, 	1172526.3524, 	1357359.9894, 	1224751.2421, 	1223220.8503, 	1222969.2115, 	1277494.8323, 	1611135.4891, 	1161879.6331, 	1163991.0554, 	1790592.1735, 	1484963.0426, 	1492203.9491, 	1483918.8533, 	1118968.9866, 	1119392.6315, 	1254532.3742, 	1116042.9183, 	1323371.8006, 	1316907.3526, 	1081335.1979, 	1617030.8268, 	1780004.9889, 	1780330.1702, 	1182205.4693, 	1182244.1878, 	1626358.0746, 	1552634.928, 	1797583.8217, 	1645553.9203, 	1234976.7443, 	953471.2228, 	952920.3105, 	1704413.6075, 	1402513.6774, 	1394777.429, 	1395193.5495, 	1416616.9123, 	1473126.0444, 	870941.284600001, 	1161092.752, 	1159143.4975, 	1162457.1503, 	1140319.2908, 	1140303.7596, 	1243313.8363, 	1287097.2188, 	1733290.6535, 	1496449.131, 	1497484.9754, 	1511209.8361, 	1179881.8404, 	1181261.7978, 	1180133.2954, 	1503492.5017, 	1503057.6765, 	1260686.4469, 	1249834.7055, 	1250235.4326, 	1260812.6496, 	1260839.0418, 	1260532.7139, 	1260833.5266, 	1255595.5106, 	1250245.5369, 	1260727.0681, 	1384219.9869, 	1387730.4063, 	1642475.3969, 	1380545.346, 	1373110.8233, 	1373478.3996, 	1066609.4377, 	1524226.689, 	1524238.1885, 	1265173.1533, 	1274467.5889, 	1265205.4496, 	1265868.157, 	1265169.9193, 	1267153.9746, 	1270401.1863, 	1233981.7567, 	1589203.5673, 	1582778.4103, 	1151381.9115, 	1156300.7505, 	1154029.8114, 	1143362.7483, 	1151385.0204, 	1223518.3274, 	1463576.7901, 	1238424.9729, 	1086627.2847, 	1086787.716, 	1086712.6659, 	1087290.2841, 	1080081.6728, 	594067.5494, 	592819.985400001, 	1237745.4764, 	899104.261299999, 	895471.2105, 	895828.8544, 	927035.694599999, 	922177.2697, 	1181781.7961, 	1119388.2831, 	1103911.5503, 	1118318.4172, 	1217137.0564, 	1216381.9379, 	1206958.6313, 	1216196.0023, 	1213460.5536, 	1215496.8889, 	1216048.5619, 	1214583.5919, 	1214477.9618, 	1216389.5876, 	1213733.4525, 	1399323.7906, 	1414952.1842, 	1399251.1055, 	1108865.7536, 	1058086.5473, 	602517.6657, 	594245.537699999, 	1235439.9391, 	1234062.5426, 	1233430.9369, 	1233120.6301, 	1235418.7948, 	1234340.2439, 	1698421.3477, 	1545910.6096, 	1548482.1898, 	1551644.4167, 	1544382.424, 	1213480.6982, 	1212776.4002, 	1212348.6633, 	1212875.9847, 	1595212.7388, 	1636522.7704, 	1478476.4446, 	1477621.3246, 	1365720.2326, 	1166021.896, 	1732517.3814, 	1612882.3065, 	926600.677100001, 	925730.397600001, 	1232356.3585, 	1238931.2171, 	1228800.5702, 	1240771.8427, 	1232349.4232, 	1470152.6785, 	1469187.937, 	1105410.5137, 	1331438.4983, 	829996.310699999, 	839776.713400001, 	829198.3181, 	829604.3169, 	1454278.3785, 	1210435.6757, 	1210881.2946, 	1146174.5004, 	1109706.7761, 	1127351.5603, 	1127663.2018, 	1125079.5506, 	1479934.4043, 	1456151.5862, 	1261270.4838, 	1263905.4097, 	1260667.3965, 	1260426.5846, 	1265769.3077, 	1266216.1886, 	1260057.0913, 	1264569.4126, 	1687146.3882
              ...
              data.table(xcoord = c(746617, 779532, 735345, 753309, 723846, 735327, 735733, 721955, 737906, 786933, 800880, 815522, 819180, 769056, 821084, 805637, 801347, 793664, 815055, 858048, 852040, 846857, 857133, 853623, 869775, 894429, 741856),
                         ycoord = c(1160354, 1169298, 1126547, 1118423, 1085785, 1058124, 1049905, 1001309, 991838, 963592, 954061, 957320, 953430, 929898, 943188, 930967, 891949, 858108, 834835, 835945, 812430, 802506, 795314, 796905, 800960, 813856, 1062111))
))
  }
  return(invisible(sim))
}
